...
void main(void)
{
    ...
    while (1)
    {
		//THIS CODE IS FOR THE SECOND PART OF THE WORKSHEET
		keypad = kpd_input(1);
		pause();
        ...
        /* THIS CODE IS FOR THE FIRST PART OF THE WORKSHEET
		keypad = read_keypad();
		...
		if(keypad!=0xFF) {
			...
			lcd_clear();
            lcd_print("Your key was:\n %d,  = Hex %X", output, output);
            printf("\n\rYour key was: %d,  = Hex %X", output, output);
            if(output == 0)printf("   **Wire Connection/XBR0 Error**   ");
		}*/
...
{
    XBR0 = 0x1F;    // NOTE: SMB on P0.6, P0.7
