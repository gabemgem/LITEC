...
#define PW_LEFT 2384
unsigned int SERVO_PW = 2764;//used to set steering PW
...
#define PW_NEUT 2764
unsigned int DRIVE_PW = 2764;//used to set the motor PW
...
void main(void)
{
	...
	while(1)
	{
		...
		if(print_count>=50) {//every 1 second
			...
			if(moving)//if the car is moving, print all data
				printf("%d,%d,%u,%u,%u,%u,%u\n\r", gx-offsetx, gy-offsety, kx, ky, (unsigned int)(ksteering*100), DRIVE_PW, SERVO_PW);
